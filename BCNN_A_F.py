...
def build_model(
   ...
   # model = keras.applications.ResNet50(nclude_top=False,input_shape=(224,224,3))
    ...
    # make (14，14，512) to (14,14,128)
    x_extractor_128 = keras.layers.Conv2D(filters=cov,kernel_size=(1,1))(x_extractor)
    shape_extractor_128 = [0,14,14,cov]
    print('------:shape_extractor_128:',shape_extractor_128[1])
    if flg_debug:
    ...
    x_detector = keras.layers.Reshape(
        [
            shape_extractor_128[1] * shape_detector[2], shape_detector[-1]
        ]
    )(x_detector)
 #   x_detector = Permute((2, 1))(x_detector)
    ...
    x_extractor_128 = keras.layers.Reshape(
        [
            shape_extractor_128[1] * shape_extractor_128[2], shape_extractor_128[-1]
        ]
    )(x_extractor_128)
...
def main(flag = 'train',flg_finetune = False,num = -1, lr = 0.1, flg_debug = True,classes_num = 32,cov=128):
    ...
    test_batches = gen.flow_from_directory(TEST_DIR, target_size=SIZE, class_mode='categorical', batch_size=BATCH_SIZE)

    CLASS =len( list(iter(tra_batches.class_indices)))
    MODE_NAME2 = str(cov)+'bcnn_advanced_final'
    if(flg_finetune):
        MODE_NAME =  str(cov)+'bcnn_advanced_finetune' + str(CLASS)
    else:
        MODE_NAME =  str(cov)+'bcnn_advanced_'+str(CLASS)
    bcnn_model = build_model(flg_debug=flg_debug, no_class=CLASS,rate_learning=lr,rate_decay_weight=1e-9,name_optimizer='sgd',flg_Freeze=True,num_Freeze=num,cov=cov)
    ...
    if flag == 'train':
        ...
        bcnn_model.fit_generator(tra_batches, steps_per_epoch=num_train_steps, epochs=100,
                                      callbacks=[early_stopping, checkpointer], validation_data=val_batches,
                                      validation_steps=num_valid_steps)
        bcnn_model.save( str(cov)+'bcnn_advanced_final.h5')
    else :
        print('\nTesting ------------')  # 对测试集进行评估
        if os.path.exists(MODE_NAME + '_best.h5'):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--cov', type=int, default=128,
    )
    FLAGS = parser.parse_args()
    if FLAGS.train:

        main(flag='train', flg_finetune=True, num=FLAGS.num, lr=FLAGS.lr, classes_num=FLAGS.classes,cov=FLAGS.cov)
    else:

        main(flag='test', flg_finetune=True, classes_num=FLAGS.classes, flg_debug=False,cov=FLAGS.cov)