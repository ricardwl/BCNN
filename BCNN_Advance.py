...
def build_model(
   ...
   # model = keras.applications.ResNet50(nclude_top=False,input_shape=(224,224,3))
    ...
    x_extractor_128 = keras.layers.Conv2D(filters=256,kernel_size=(1,1))(x_extractor)
    shape_extractor_128 = [0,14,14,256]
...
def main(flag = 'train',flg_finetune = False,num = -1, lr = 0.1, flg_debug = True,classes_num = 32):
    ...
    MODE_NAME2 = 'bcnn_advanced_final'
    if(flg_finetune):
        MODE_NAME = 'bcnn_advanced_finetune' + str(CLASS)
    else:
        MODE_NAME = 'bcnn_advanced_'+str(CLASS)
    ...
    if flag == 'train':
        ...
        bcnn_model.save('bcnn_advanced_final.h5')
    else :
        print('\nTesting ------------')  # 对测试集进行评估
        if os.path.exists(MODE_NAME + '_best.h5'):
